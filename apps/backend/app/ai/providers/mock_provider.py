...
class MockAIProvider(AIProvider):
    async def chat(self, messages: List[Dict[str, Any]], tools: Optional[List[Dict[str, Any]]] = None) -> AIResponse:
        ...
        if "email importante" in last_msg or "emails importantes" in last_msg or "urgente" in last_msg and "email" in last_msg:
            return AIResponse(
                content=None,
                tool_calls=[ToolCall(id=str(uuid.uuid4()), name="list_important_emails", arguments={"limit": 5})],
                usage={"prompt_tokens": 10, "completion_tokens": 15, "total_tokens": 25}
            )
        elif "email não lido" in last_msg or "emails não lidos" in last_msg or "emails pendentes" in last_msg:
            return AIResponse(
                content=None,
                tool_calls=[ToolCall(id=str(uuid.uuid4()), name="get_unread_emails", arguments={"limit": 5})],
                usage={"prompt_tokens": 10, "completion_tokens": 15, "total_tokens": 25}
            )
        elif "resumo" in last_msg and "email" in last_msg or "panorama" in last_msg and "email" in last_msg:
            return AIResponse(
                content=None,
                tool_calls=[ToolCall(id=str(uuid.uuid4()), name="get_email_summary", arguments={})],
                usage={"prompt_tokens": 10, "completion_tokens": 15, "total_tokens": 25}
            )
        elif "procur" in last_msg and "email" in last_msg or "pesquis" in last_msg and "email" in last_msg or "email sobre" in last_msg:
            # Extract query after 'sobre' or generic search
            query = "pagamento"
            if "sobre " in last_msg:
                query = last_msg.split("sobre ")[1].split()[0].strip()
            return AIResponse(
                content=None,
                tool_calls=[ToolCall(id=str(uuid.uuid4()), name="search_emails", arguments={"query": query})],
                usage={"prompt_tokens": 10, "completion_tokens": 15, "total_tokens": 25}
            )
        ...
        return AIResponse(
            content="Olá! Eu sou o assistente do Resolva. Posso consultar seus e-mails, tarefas, finanças e estudos ou ajudar a planejar o seu dia.",
            tool_calls=None,
            usage={"prompt_tokens": 10, "completion_tokens": 20, "total_tokens": 30}
        )
