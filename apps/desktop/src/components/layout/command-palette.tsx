...
export function CommandPalette({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  const quickCommands = [
    { id: "new-task", title: "Criar nova tarefa", category: "Ações", icon: CheckSquare, color: "text-emerald-400", action: () => { onClose(); setActiveQuickModal("task"); } },
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="lg">
      <div className="flex flex-col h-[520px] -m-6" onKeyDown={handleKeyDown}>
        <div className="flex items-center px-4 py-3.5 border-b border-surface-700/80 bg-surface-900/60 backdrop-blur-md">
          <Search className="h-5 w-5 text-accent-400 mr-3" />
          <input 
            autoFocus
                ...
                return (
                  <button 
                    ...
                    className={`w-full flex items-center px-3 py-2.5 rounded-lg text-left group transition-all outline-none ${
                      isSelected ? "bg-accent-600/20 border border-accent-500/40 text-white" : "hover:bg-surface-800 text-surface-200"
                    }`}
                    onClick={() => handleSelectSearchItem(item)}
                  >
                    <div className="h-8 w-8 rounded-md bg-surface-800 flex items-center justify-center mr-3 border border-surface-700/50 text-accent-400">
                    ...
                    <ArrowRight className="h-4 w-4 text-surface-500 opacity-60" />
                  </button>
                );
              })}
            </div>
              ...
              {groupedCategories.map((cat) => {
                ...
                return (
                  <div key={cat} className="space-y-1">
                    <div className="px-2 py-1 text-[11px] font-bold text-surface-400 uppercase tracking-wider">
                      {cat}
                    </div>
                    {items.map((cmd) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center px-3 py-2 rounded-lg text-left transition-colors ${
                            isSelected ? "bg-accent-600/20 border border-accent-500/40 text-white" : "hover:bg-surface-800/70 text-surface-200"
                          }`}
                          onClick={cmd.action}
                        >
                          <div className="h-7 w-7 rounded-md bg-surface-800/80 flex items-center justify-center mr-3 text-surface-300 border border-surface-700/50">
                            <cmd.icon className={`h-3.5 w-3.5 ${cmd.color}`} />
                          </div>
                          <span className="text-sm font-medium flex-1">{cmd.title}</span>
                          <ChevronRight className="h-3.5 w-3.5 text-surface-500 opacity-40" />
                        </button>
                      );
                    })}
                  </div>
