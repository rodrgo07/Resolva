  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/40 pb-5">
        <div>
          <h1 className="text-2xl font-bold text-text-primary tracking-tight">Automações & Rotinas</h1>
        ...
        <form onSubmit={handleSaveAutomation} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {triggerType === "SCHEDULE" && (
              <div>
                <label className="text-xs font-semibold text-text-secondary mb-1.5 block">Horário (HH:MM)</label>
                <Input
                  type="time"
                  value={scheduleTime}
          ...
          {actionType === "START_STUDY_SESSION" && (
            <div className="p-3 rounded-lg border border-border bg-surface/40">
              <label className="text-xs font-semibold text-text-secondary mb-1 block">Duração em Minutos</label>
              <Input
                type="number"
                min={5}
                max={120}
          ...
          {(actionType === "CREATE_NOTIFICATION" || actionType === "SHOW_AGENT_MESSAGE") && (
            <div className="space-y-3 p-3 rounded-lg border border-border bg-surface/40">
              <div>
                <label className="text-xs font-semibold text-text-secondary mb-1 block">Título</label>
